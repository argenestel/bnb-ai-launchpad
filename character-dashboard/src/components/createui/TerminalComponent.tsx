...
const ASCIISpinner: React.FC<{ status: BuildStage["status"] }> = ({ status }) => {
  const frames = ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"];
...
const BuildStage: React.FC<BuildStage> = ({ stage, status, logs = [] }) => {
  ...
  return (
    <div className="space-y-1 mb-3">
      <div className={cn("font-mono flex items-center", getStatusColor(status))}>
...
const TerminalComponent: React.FC<TerminalComponentProps> = ({
  ...
  const defaultStages: BuildStage[] = [
    ...
    },
    {
      ...
      status: "running",
      logs: ["• Optimizing build settings", "• Configuring environment variables"]
    },
    {
